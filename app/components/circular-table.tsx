...
import { Crown, Flame, Scissors, Spade, X, Plus, Minus, Check, Users } from "lucide-react";
...
function getRankStyle(rankIndex: number, isActive: boolean) {
  const styles = [
    { bg: "bg-gradient-to-br from-amber-400 to-amber-600", text: "text-amber-950", border: "border-amber-500/30", glow: "shadow-amber-500/20" },
    { bg: "bg-gradient-to-br from-slate-300 to-slate-500", text: "text-slate-950", border: "border-slate-400/30", glow: "shadow-slate-400/20" },
    { bg: "bg-gradient-to-br from-orange-300 to-orange-500", text: "text-orange-950", border: "border-orange-400/30", glow: "shadow-orange-400/20" },
    { bg: "bg-gradient-to-br from-rose-400 to-rose-600", text: "text-rose-950", border: "border-rose-400/30", glow: "shadow-rose-400/20" },
  ];
      ...
      <div className="flex items-start justify-between gap-2">
        <div className="flex flex-col gap-1">
          ...
          <div className={`
            ...
            ${isFixed ? rankStyle.bg : "bg-muted text-muted-foreground"}
      ...
      {(chatHeo.asChatter.length > 0 || chatHeo.asVictim.length > 0) && (
        <div className="mt-3 flex flex-wrap gap-1">
          {chatHeo.asChatter.map((c) => {
            ...
            return (
              <div key={c.id} className="flex items-center gap-1 rounded-xl border border-chart-2/30 bg-chart-2/15 px-2 py-1">
                <Scissors className="size-3 text-chart-2" />
                {(c.heo.do ?? 0) > 0 && (
                ...
                {(c.heo.den ?? 0) > 0 && (
                  <span className="rounded-full bg-foreground px-1.5 py-0.5 text-[10px] font-black text-background">
                    {c.heo.den}đ
                  </span>
                )}
                <span className="text-xs font-black text-chart-2">+{pts}</span>
              </div>
            );
          })}
          {chatHeo.asVictim.map((c) => {
            const pts = (c.heo.do ?? 0) * gameConfig.heoDoPoints + (c.heo.den ?? 0) * gameConfig.heodenPoints;
            return (
              <div key={c.id} className="flex items-center gap-1 rounded-xl border border-destructive/20 bg-destructive/10 px-2 py-1">
                <Scissors className="size-3 text-destructive" />
                {(c.heo.do ?? 0) > 0 && (
                  <span className="rounded-full bg-red-500 px-1.5 py-0.5 text-[10px] font-black text-white">
                    {c.heo.do}đ
                  </span>
                ...
                <span className="text-xs font-black text-destructive">-{pts}</span>
              </div>
            );
          })}
        </div>
  ...
  return (
    <div className="relative w-full py-4">
      ...
      <div className="absolute inset-0 flex items-center justify-center pointer-events-none">
        <div 
          className="w-64 h-64 rounded-full border-2 border-dashed border-border/30"
          style={{
            background: `radial-gradient(circle, transparent 60%, var(--background) 70%)`
          }}
        />
      </div>
      ...
      <div className="relative z-10 flex items-center justify-center pb-4">
        <Button
          ...
          className={`
            flex flex-col items-center justify-center gap-1 rounded-full shadow-xl transition-all duration-300
            ${disabledSaveButton 
      ...
      <div className="relative z-20 mx-auto w-[340px] h-[340px]">
        {players.map((player, idx) => {
          ...
          const khapPts = isKhapWinner && khapCount > 0
          ...
          const adjustedLeft = Math.max(0, Math.min(340 - cardWidth, left));
          const adjustedTop = Math.max(0, Math.min(340 - cardHeight, top));
      ...
      <div className="mt-4 flex flex-col items-center gap-2">
        <div className="flex items-center gap-2 text-sm">
          ...
          <span className="text-muted-foreground">
            {selectCounter}/{activeNhot ? requiredSelections : players.length} đã chọn
          </span>
        </div>
        <div className="h-2 w-48 overflow-hidden rounded-full bg-muted">
          <div 
            ...
            style={{ 
              width: `${((selectCounter) / (activeNhot ? requiredSelections : players.length)) * 100}%` 
            }}
          />
        </div>
      </div>
        ...
        <div className="mt-4 flex items-center justify-center gap-4">
          {khapWinner && khapCount > 0 && (
            <div className="flex items-center gap-2 rounded-2xl border border-chart-4/30 bg-chart-4/10 px-4 py-2">
              ...
              <span className="text-sm font-bold text-chart-4">Khạp x{khapCount}</span>
              <div className="flex items-center gap-1">
                <button
          ...
          {sanhWinner && (
            <div className="flex items-center gap-2 rounded-2xl border border-chart-1/30 bg-chart-1/10 px-4 py-2">
              ...
              <span className="text-sm font-bold text-chart-1">Sảnh x{accumulated.sanh}</span>
            </div>
          )}
        </div>
      )}
